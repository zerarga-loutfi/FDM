...
// g++ -O2 -std=c++17 -pthread quota_stratified_linked_sample_disque.cpp -o quota_stratified_linked_sample_disque
//
/*
./quota_stratified_linked_sample_disque input.txt output.txt 1000000
...
struct Concept {
  ...
  int card = 0;
  int64_t file_offset = -1;   // byte offset in input file (replaces raw_line to save RAM)
...
static Concept parse_intent_line(const std::string& line, int num_attributes, int order_id, int64_t offset) {
  Concept c;
...
int main(int argc, char** argv) {
  ...
  while (std::getline(fin, line)) {
    ...
    int64_t offset = (int64_t)fin.tellg() - (int64_t)line.size() - 1; // -1 for '\n'
    Concept c = parse_intent_line(line, num_attributes, order_id++, offset);
  ...
  // Write output: reopen input file to read lines by offset (avoids keeping raw_line in RAM)
  std::ifstream fin_out(input_file, std::ios::binary);
  if (!fin_out) {
    std::cerr << "Error: cannot reopen input for output writing\n";
    return 1;
  }
  ...
  for (size_t c = 0; c < selected_by_card.size(); ++c) {
    for (const Concept* p : selected_by_card[c]) {
      fin_out.seekg(p->file_offset);
      std::string raw;
      std::getline(fin_out, raw);
      fout << raw << "\n";
    }
  }
  fin_out.close();
